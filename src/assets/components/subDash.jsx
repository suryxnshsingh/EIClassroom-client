...
const List = ({ subjectCode }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`http://${process.env.BACKEND_URL}:8080/api/operation/sheets?subjectCode=${subjectCode}`);
...
const AddExamSchema = ({ setSchema, subjectCode }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://${process.env.BACKEND_URL}:8080/api/operation/co-form`, dataToSubmit);
...
const AddStudentPopup = ({ setCreate, subjectCode }) => {
  ...
      try {
        const response = await axios.post(`http://${process.env.BACKEND_URL}:8080/api/operation/submit-form`, formData, {
          headers: {
  ...
  const downloadExcel = (subjectCode) => {
    axios.get(`http://${process.env.BACKEND_URL}:8080/api/operation/download-sheets?subjectCode=${subjectCode}`, {
  ...
  const downloadMST1 = (subjectCode) => {
    axios.get(`http://${process.env.BACKEND_URL}:8080/api/operation/downloadmst1/${subjectCode}`, {
      responseType: 'blob', // Important to set response type as blob for file download
    })
    .then((response) => {
      // Create a URL for the blob
  ...
  const downloadMST2 = (subjectCode) => {
    axios.get(`http://${process.env.BACKEND_URL}:8080/api/operation/downloadmst2/${subjectCode}`, {
      responseType: 'blob', // Important to set response type as blob for file download
    })
    .then((response) => {
      // Create a URL for the blob
  ...
  const downloadExcelEnd = (subjectCode) => {
    axios.get(`http://${process.env.BACKEND_URL}:8080/api/operation/end-excel/${subjectCode}`, {
      responseType: 'blob',
    })
    .then((response) => {
      const url = window.URL.createObjectURL(new Blob([response.data]));
  ...
  const downloadCOSheet = (subjectCode) => {
    axios.get(`http://${process.env.BACKEND_URL}:8080/api/operation/generate-co-attainment/${subjectCode}`, {
      responseType: 'blob',
    })
    .then((response) => {
      const url = window.URL.createObjectURL(new Blob([response.data]));
